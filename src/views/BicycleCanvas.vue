<template>
  <div>
    <div>hello! my first bicycle animation</div>
    <a href="https://codepen.io/meibin08/pen/xyXmNq?editors=0010">出处</a>
  </div>
</template>
  ...
  export default {
    data() {
      return {
        ...
        canvasH: 420, // 画布的高
        proportion: 4, // 未知
        canvasBg: '#f9f8ee', // 画布背景色
        color: '#301926', // 车的颜色
        gearColor: '#160a13', // 未知
        wheelBorder: 14, // 未知
        wheelPos: [], // 未知
        axisDotPos: [], // 未知
        edge: 360, // this.canvasW - this.oneCent; //最大值
        animateNum: 0, // 动画计数
      }
    },
    computed:{
      wheelRadius() {
        return this.canvasH * .1814;
      },
      oneCent() {
        return this.canvasW * 0.245;
      }
    },
    mounted () {
      //绘制圆形的方法
      CanvasRenderingContext2D.prototype.roundRect = function (x, y, w, h, r) {
        var min_size = Math.min(w, h) // 求两者之间的最小值
        if (r > min_size / 2) r = min_size / 2
        // 开始绘制
        this.beginPath() // 起始一条路径，或者重置当前路径
        this.moveTo(x + r, y) // 把路径移动到画布中的指定点，不创建线条
        this.arcTo(x + w, y, x + w, y + h, r) // 创建两点之间的弧线或者曲线
    ...
    methods: {
      ...
      run() {
        this.horizon();
        this.ctx.restore(); // 返回之前保存过的路径跟属性
        let _requestAnimatuionFrame_ = window.requestAnimationFrame || window.webkitRequestAnimationFrame;
        // window.requestAnimationFrame方法告诉浏览器您希望执行动画并请求浏览器在下一次重绘之前调用指定的函数来更新动画。
        // 该方法使用一个回调函数作为参数，这个回调函数会在浏览器重绘之前调用。
        this.animateNum += 3;
        _requestAnimatuionFrame_(() => this.run());
       ...
       */
        ...
        this.ctx.clearRect(0, 0, this.canvasW, this.canvasH); // 在给定的矩形内清除指定的像素
        let horizonX = 0, horizonY = this.canvasH - 100;
        this.ctx.beginPath(); // 起始一条路径或者重置当前路径
        this.ctx.strokeStyle = this.color; // 设置或者返回用于笔触的颜色、渐变或者模式
        this.ctx.lineWidth = 6; // 设置或者返回当前的线条的宽度
      ...
      wheel(){
        ...
        this.wheelPos.map((wheelItem,v)=>{
          ...
          if(v === 0){
            ...
            [
              {lineW:2,radius:scaleMultiple*.6,sAngle:this.getRads(-135+speed1) , eAngle:this.getRads(110+speed1)},
              {lineW:1.2,radius:scaleMultiple*.45,sAngle:this.getRads(45+speed2) , eAngle:this.getRads(-50+speed2)}
            ].map((k,v)=>{
              this.ctx.beginPath();
              this.ctx.lineCap="round";
            ...
            Array.from({length:3}).map((k,v)=>{
              ...
              this.ctx.arc(wheelItemX,wheelItemY,radius,this.getRads(sAngle+speed1),this.getRads(eAngle+speed1),false);
              this.ctx.stroke();
              this.ctx.closePath();
              ...
              if(v<2){
                ...
                this.ctx.strokeStyle = _color_;
                this.ctx.lineWidth=3;
                this.ctx.arc(wheelItemX,wheelItemY,radiusSmaller,this.getRads(sAngleSmaller+speed2),this.getRads(eAngleSmaller+speed2),false);
                this.ctx.stroke();
                this.ctx.closePath();
              }
              this.ctx.restore();
